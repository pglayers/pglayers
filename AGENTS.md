...
8. **`extension.conf` PG_CONF field** -- If the extension requires
   ...
   > **Single source of truth.** The whole combined-image runtime
   > configuration -- `extension_control_path`/`dynamic_library_path`
   > (PG18 isolated), `shared_preload_libraries`,
   > `session_preload_libraries` (pgtt), `max_worker_processes`, every
   > extension's `PG_CONF`, and the pgsodium/documentdb path fixups -- is
   > generated by **`scripts/combined-config.sh`**. Both the shipped-image
   > builder (`Makefile` `image` target) and the test harness
   > (`tests/test-layers.sh`) call it, so the image users run and the
   > image CI tests cannot drift. Never re-implement any of these settings
   > inline in the Makefile or the test harness -- add them to
