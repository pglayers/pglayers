...
# Generate a test Dockerfile
{
    ...
    # documentdb gateway config (only if present)
    if printf '%s\n' "${EXTENSIONS[@]}" | grep -qx documentdb; then
        echo "RUN echo \"documentdb_gateway.database = 'postgres'\" >> /usr/share/postgresql/postgresql.conf.sample"
        ...
        echo "RUN echo \"documentdb_gateway.setup_configuration_file = '/etc/documentdb/gateway_config.json'\" >> /usr/share/postgresql/postgresql.conf.sample"
    fi
} > "${TMPDIR}/Dockerfile"
