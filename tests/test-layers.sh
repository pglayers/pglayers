...
# Generate a test Dockerfile
{
    ...
    # Runtime configuration (extension_control_path, dynamic_library_path,
    # shared_preload_libraries, session_preload_libraries, max_worker_processes,
    # per-extension PG_CONF GUCs, pgsodium/documentdb fixups) is generated by the
    # SAME script the shipped image uses (make image), so the tested image and
    # the shipped image can't drift.
    ./scripts/combined-config.sh "$PG" "${EXTENSIONS[@]}"
} > "${TMPDIR}/Dockerfile"
