...
-- Test: GeoJSON output
SELECT CASE
    WHEN ST_AsGeoJSON(ST_MakePoint(1, 2))::jsonb->>'type' = 'Point'
...
INSERT INTO test_postgis (geom) SELECT ST_SetSRID(ST_MakePoint(random()*360-180, random()*180-90), 4326) FROM generate_series(1,100);
CREATE INDEX ON test_postgis USING gist (geom);
SELECT CASE
    WHEN (SELECT count(*) FROM test_postgis WHERE ST_DWithin(geom, ST_SetSRID(ST_MakePoint(0,0), 4326), 50)) >= 0
